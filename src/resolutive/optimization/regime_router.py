...
class ResolutiveRegimeRouter:
    ...
    def _diagnose(self, objective: Objective, dimension: int, lo: float, hi: float,
        ...
        diag = RegimeDiagnostics(noise_ratio, ruggedness, anisotropy, curvature_proxy, probs, selected)
        return diag, used
    ...
    def minimize(self, objective: Objective, *, dimension: int,
        ...
        result = optimizer.minimize(objective, dimension=dimension, bounds=(lo, hi), budget=remaining, seed=seed)
        return OptimizationResult(result.x, result.fun, used + result.evaluations, seed,
