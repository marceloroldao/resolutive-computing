    ...
    def minimize(
        ...
        *,
        ...
        while used + pop <= budget - collapse_budget:
            ...
            if not escape and stall >= 10 and relative_improvement < 1e-5:
                escape = True
                escape_left = 8
            ...
            if not escape:
                ...
                for ids in np.array_split(np.arange(pop), 4):
                    ...
                    for idx in ids:
                        ...
                        if anisotropy > 80.0:
                            step += 0.08 * span * (1.0 if rng.random() < 0.5 else -1.0) * eigvecs[:, -1]
                ...
                if escape_left <= 0:
                    ...
                    for idx in worst:
                        if used >= budget - collapse_budget:
                            break
                        x[idx] = np.clip(best + rng.normal(0.0, 0.03 * span, dimension), lo, hi)
